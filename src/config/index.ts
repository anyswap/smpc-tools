...
const netConfig: ConFig = chainInfo[ENV]
  ...
  : chainInfo[INIT_NODE];
// console.log(ENV)
const INITBRIDGE = getInitBridgeChain(
  netConfig.bridgeInitChain,
  netConfig.bridgeInitToken
);
    ...
    if (
      ...
      (value === "BASECURRENCY" ||
        (value === "W" + this.getCurChainInfo(chainId).symbol &&
          this.getCurChainInfo(chainId).nativeToken))
    ) {
      if (type) {
        return (
          (this.getCurChainInfo(chainId).symbolName ??
            this.getCurChainInfo(chainId).name) + "(Router)"
        );
      } else {
