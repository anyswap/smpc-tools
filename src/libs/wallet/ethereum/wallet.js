...
Wallet.prototype.setTokens = function () {
  ...
  var defaultTokensAndNetworkType =
    globalFuncs.getDefaultTokensAndNetworkType();
  ...
  for (var e = 0; e < storedTokens.length; e++) {
    if (
      globalFuncs.doesTokenExistInDefaultTokens(
        storedTokens[e],
        defaultTokensAndNetworkType
      )
    ) {
...
function removeConflictingTokensFromLocalStorage(
  conflictLocalTokens,
  localTokens
) {
...
function removeAllTokenConflicts(conflictWithDefaultTokens, localTokens) {
  var deConflictedTokens = removeConflictingTokensFromLocalStorage(
    conflictWithDefaultTokens,
    localTokens
  );
  ...
  ajaxReq.getBalance(parentObj.getAddressString(), function (data) {
    if (data.error) parentObj.balance = data.msg;
      ...
      ajaxReq.getETHvalue(function (data) {
        ...
        parentObj.usdBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.usd
        );
        parentObj.gbpBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.gbp
        );
        parentObj.eurBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.eur
        );
        parentObj.btcBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.btc
        );
        parentObj.chfBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.chf
        );
        parentObj.repBalance = etherUnits.toFiat(
          parentObj.balance,
          "ether",
          data.rep
        );
...
Wallet.getWalletFromPrivKeyFile = function (strjson, password) {
  ...
  else if (jsonArr.publisher == config.AppName)
