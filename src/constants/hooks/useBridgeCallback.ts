  ...
  return useMemo(() => {
    ...
    return {
              ...
              try {
                ...
                addTransaction(txReceipt, {
                  summary: `Cross bridge ${inputAmount.toSignificant(
                    6
                  )} ${config.getBaseCoin(inputCurrency?.symbol, chainId)}`,
      ...
      inputError: sufficientBalance
        ? undefined
        : t("Insufficient", { symbol: inputCurrency?.symbol }),
    };
  }, [
    bridgeContract,
    chainId,
          ...
          ? async () => {
              try {
                ...
                addTransaction(txReceipt, {
                  ...
                  } ${inputAmount.toSignificant(6)} ${config.getBaseCoin(
...
export function useCrossBridgeCallback(
  ...
  const addTransaction = useTransactionAdder();
  const contractBTC = useSwapBTCContract(
    isAddress(inputToken) ? inputToken : undefined
  );
  const contractETH = useSwapETHContract(
    isAddress(inputToken) ? inputToken : undefined
  );
  return useMemo(() => {
    // console.log(inputToken)
    if (!chainId || !toAddress || !toChainID || !library || !receiveAddress)
      return NOT_APPLICABLE;
    // console.log(typedValue)
    // console.log(toChainID)
              ...
              try {
                ...
                if (txnsType === "swapin") {
                  if (isAddress(inputToken)) {
                    if (contractETH) {
                      txReceipt = await contractETH.transfer(
                        toAddress,
                        `0x${inputAmount.raw.toString(16)}`
                      );
                    } else {
                    ...
                    const hash = await library.send("eth_sendTransaction", [
                      data,
                    ]);
                    txReceipt =
                      hash && hash.toString().indexOf("0x") === 0
                        ? { hash }
                        : "";
                  }
                } else {
                  ...
                  if (toChainID && isNaN(toChainID)) {
                    if (contractBTC) {
                      txReceipt = await contractBTC.Swapout(
                        `0x${inputAmount.raw.toString(16)}`,
                        toAddress
                      );
                    } else {
                    ...
                    if (contractETH) {
                      txReceipt = await contractETH.Swapout(
                        `0x${inputAmount.raw.toString(16)}`,
                        toAddress
                      );
                    } else {
                ...
                if (txData.hash && account) {
                  ...
                  addTransaction(txData, {
                    summary: `Cross bridge ${inputAmount.toSignificant(
                      6
                    )} ${config.getBaseCoin(symbol, chainId)}`,
                    value: inputAmount.toSignificant(6),
                    toChainId: toChainID,
...
export function useTerraCrossBridgeCallback(
  ...
  const terraToken =
    inputCurrency?.address?.indexOf("terra") === 0
  ...
  const inputAmount = useMemo(
    () =>
      inputCurrency
        ? tryParseAmount3(typedValue, inputCurrency?.decimals)
        : undefined,
    [inputCurrency, typedValue]
  );
  // console.log(inputCurrency)
  ...
  useEffect(() => {
    // console.log(connectedWallet)
    if (connectedWallet?.walletAddress && toAddress && useNnit && inputAmount) {
      // console.log(inputAmount)
      getTerraFeeList(
  ...
  const fetchBalance = useCallback(() => {
    if (terraToken && connectedWallet) {
      ...
      getTerraBalances({
        ...
        // console.log(res)
        const bl: any =
          res[terraToken] && inputCurrency
            ? new Fraction(
                JSBI.BigInt(res[terraToken]),
                JSBI.exponentiate(
                  JSBI.BigInt(10),
  ...
  return useMemo(() => {
    ...
    try {
      sufficientBalance =
        ...
        typedValue &&
        balance &&
        Number(balance?.toSignificant(inputCurrency?.decimals)) >
          Number(typedValue);
    } catch (error) {
      console.log(error);
    }
        ...
        ? async () => {
            try {
              ...
              if (txReceipt) {
                ...
                addTransaction(txData, {
                  summary: `Cross bridge ${typedValue} ${config.getBaseCoin(
                    inputCurrency?.symbol,
                    chainId
                  )}`,
                  value: typedValue,
