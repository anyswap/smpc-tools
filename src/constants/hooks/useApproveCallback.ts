    ...
    return tokenContract
      ...
      .then((response: TransactionResponse) => {
        ...
        addTransaction(response, {
          summary:
            config.getBaseCoin(amountToApprove.currency.symbol, chainId) +
            " approved, you can continue the cross chain transaction",
...
export function useNonApproveCallback(token: any, spender: any, symbol: any) {
  const { account, chainId } = useActiveWeb3React();
  const addTransaction = useTransactionAdder();
