...
export function useNFT721Callback(
  ...
  return useMemo(() => {
    ...
    const sufficientBalance =
      ethBalance && nftBalance?.toLowerCase() === account?.toLowerCase();
          ...
          ? async () => {
              try {
                ...
                const txReceipt = await contract.nft721SwapOut(
                  ...[inputCurrency?.address, toAddress, tokenid, toChainID],
                  { value: fee }
                );
                addTransaction(txReceipt, {
                  summary: `Cross bridge ${tokenid} ${config.getBaseCoin(
...
export function useNFT1155Callback(
    ...
    if (contract1155 && tokenid) {
      contract1155
        ...
        .then((res: any) => {
          ...
          if (res) {
            setNftBalance(
              tryParseAmount2(res.toString(), inputCurrency?.decimals ?? 0)
            );
          } else {
            setNftBalance("");
          }
  ...
  const inputAmount = useMemo(
    () =>
      inputCurrency
        ? tryParseAmount2(amount, inputCurrency?.decimals ?? 0)
        : "",
  ...
  return useMemo(() => {
    ...
    const sufficientBalance =
      ...
      inputAmount &&
      nftBalance &&
      !nftBalance.lessThan(inputAmount);
