...
export function useTerraBaseBalance() {
  ...
  const getTerraBaseBalances =
    useCallback(async (): Promise<BalanceListType> => {
...
export function useTerraTokenBalances() {
  const connectedWallet = useConnectedWallet();
  const { fetchQuery } = useMantle();
  const getTerraTokenBalances = useCallback(
    async ({
      terraWhiteList,
    }: {
...
const useTerraBalance = (): {
  ...
  const getTerraBankBalances = useCallback(
    async (account?: any): Promise<BalanceListType> => {
      const useAccount = account ? account : connectedWallet?.walletAddress;
      if (!useAccount) return {};
      const fetchResult = await fetchQuery({
        query: bankBalanceQuery,
  ...
  const getTerraBalances = useCallback(
    async ({
      terraWhiteList,
      account,
    }: {
